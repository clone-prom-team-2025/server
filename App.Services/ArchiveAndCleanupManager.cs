...
public class ArchiveAndCleanupManager : IArchiveAndCleanupManager
{
    ...
    public ArchiveAndCleanupManager(MongoDbContext context, ILogger<ArchiveAndCleanupManager> logger, IMapper mapper,
        IMongoClient client, IFileService fileService)
    {
        _logger = logger;
        _mapper = mapper;
        _client = client;
    ...
    public async Task PermanentlyDeleteProductAsync(ObjectId id)
    {
        ...
        try
        {
            ...
            if (product == null) _logger.LogWarning("No archived product found with Id {id}", id);
            ...
            _logger.LogInformation(
                $"Permanently removed {nameof(Product)}, {nameof(ProductMedia)} and {nameof(ProductReview)} for ProductId {id}");
    ...
    public async Task RestoreProductAsync(ObjectId id)
    {
        ...
        var archivedProductCollection =
            await _productArchiveCollection.Find(archivedProductFilter).FirstOrDefaultAsync();

        var archivedProductMediaFilter = Builders<ProductMediaArchive>.Filter.Eq(d => d.ProductId, id);
        var archivedProductMediaCollection =
            await _productMediaArchiveCollection.Find(archivedProductMediaFilter).ToListAsync();

        var archivedProductReviewFilter = Builders<ProductReviewArchive>.Filter.Eq(d => d.ProductId, id);
        var archivedProductReviewCollection =
            await _productReviewArchiveCollection.Find(archivedProductReviewFilter).ToListAsync();
        ...
        try
        {
            ...
            if (productMediaCollection.Count > 0)
                await _productMediaArchiveCollection.DeleteManyAsync(session,
                    Builders<ProductMediaArchive>.Filter.Eq(d => d.ProductId, id));
            if (productReviewCollection.Count > 0)
                await _productReviewArchiveCollection.DeleteManyAsync(session,
                    Builders<ProductReviewArchive>.Filter.Eq(d => d.ProductId, id));
    ...
    public async Task CleanupOldArchivedProductsAsync()
    {
        ...
        foreach (var p in oldProducts) await PermanentlyDeleteProductAsync(p.ProductId);
    }
    ...
    public async Task<ProductArchive> GetArchivedProductsAsync(ObjectId id)
    {
        ...
        var archivedProductCollection =
            await _productArchiveCollection.Find(p => p.ProductId == id).FirstOrDefaultAsync();
